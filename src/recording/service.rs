...
use crate::{
    config::RecordingNfoStyle, recording_rules, twitch_auth::TwitchAuthService,
    util::channel::normalize_channel_login, util::time::now_unix_secs,
};
...
use super::{
    ...
    nfo::*,
    playback::{rebuild_hls_playlist, write_playback_assets},
    process::{ActiveProcess, reconcile_exited_recordings},
    types::*,
};
...
impl RecordingService {
    ...
    async fn reconcile_exited_recordings(&self) {
        reconcile_exited_recordings(
            ...
            self.write_nfo,
            self.nfo_style,
            self.chapter_min_gap_secs,
            self.chapter_change_confirmations,
            &self.ffmpeg_path,
            &self.twitch,
        )
        .await;
