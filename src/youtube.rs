...
use crate::{
   ...
   youtube_progress::{
      YoutubeProgressStore,
      YoutubeWatchProgressEntry,
   },
   youtube_quality::{
      QualityObservation,
      QualityObservedResponse,
      get_video_quality_observed,
      get_video_quality_stream,
      observe_quality_from_request,
   },
};
...
impl YoutubeState {
   ...
   pub(crate) const fn invidious_client(&self) -> Option<&InvidiousClient> {
...
const fn default_recent_max_results() -> u32 {
   25
}
...
async fn put_video_progress(
   ...
   let previous_completed = saved.previous.is_some_and(|entry| entry.completed);
...
/// Shared helper to proxy image requests through Invidious with `YouTube` CDN
/// fallback
...
async fn fetch_youtube_cdn_thumbnail(client: &InvidiousClient, video_id: &str) -> Response {
   let youtube_url = format!("https://i.ytimg.com/vi/{video_id}/hqdefault.jpg");
   ...
   // Cache for 24 hours since thumbnails rarely change (or use YouTube's cache
   // headers)
   headers.insert(
      "cache-control",
      HeaderValue::from_static("public, max-age=86400"),
...
async fn get_thumbnail(
   ...
   let invidious_url = format!("{base_url}/vi/{video_id}/hqdefault.jpg");
...
async fn proxy_video_segment(
   ...
   let mut upstream_url = format!("{base_url}/companion/{trimmed_path}");
...
async fn proxy_companion_api(
   ...
   let trimmed_path = path.trim_start_matches('/');
   let mut upstream_url = format!("{base_url}/companion/api/{trimmed_path}");
   if let Some(query_string) = raw_query
      && !query_string.is_empty()
   {
      upstream_url.push('?');
