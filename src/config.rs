...
impl FromStr for StreamResolverMode {
   ...
   fn from_str(s: &str) -> Result<Self, Self::Err> {
      match s.trim().to_ascii_lowercase().as_str() {
         ...
         _ => {
            Err(format!(
               "invalid STREAM_RESOLVER_MODE: '{s}'. Must be one of: auto, native, streamlink"
            ))
         },
      }
   }
...
impl FromStr for StreamDeliveryMode {
   ...
   fn from_str(s: &str) -> Result<Self, Self::Err> {
      match s.trim().to_ascii_lowercase().as_str() {
         ...
         _ => {
            Err(format!(
               "invalid STREAM_DELIVERY_MODE: '{s}'. Must be one of: cdn_first, relay"
            ))
         },
      }
   }
...
impl AppConfig {
   pub fn from_env() -> Result<Self, AppError> {
      let bind_addr =
         parse_socket_addr("BIND_ADDR")?.unwrap_or_else(|| SocketAddr::from(([0, 0, 0, 0], 8080)));
      ...
      // Invidious config is optional - YouTube mode will be disabled if not
      // configured
...
///
/// 1. Normal mode: Direct Invidious API auth using `Authorization: Bearer
///    <token>`. Requires: `INVIDIOUS_BASE_URL` + `INVIDIOUS_TOKEN`
///
...
///    so the Invidious session must be sent via the `SID` cookie instead of the
///    Bearer `Authorization` header. Requires: `INVIDIOUS_BASE_URL` +
///    `INVIDIOUS_BASIC_AUTH_USER` + `INVIDIOUS_BASIC_AUTH_PASSWORD` +
///    `INVIDIOUS_SID`
///
...
fn parse_invidious_config() -> Result<Option<InvidiousConfig>, AppError> {
   ...
   if has_basic_auth_user != has_basic_auth_password {
      return Err(AppError::Config(
         "INVIDIOUS_BASIC_AUTH_USER and INVIDIOUS_BASIC_AUTH_PASSWORD must both be set or neither"
            .to_string(),
      ));
   }
   ...
   if basic_auth_configured && sid_cookie.is_none() {
      return Err(AppError::Config(
         "INVIDIOUS_SID is required when INVIDIOUS_BASIC_AUTH_USER and \
          INVIDIOUS_BASIC_AUTH_PASSWORD are set"
            .to_string(),
      ));
   }
   ...
   let token = match env::var("INVIDIOUS_TOKEN") {
      ...
      _ => {
         if !basic_auth_configured {
            // If base_url is set but token is not, and we're not in Basic auth mode, that's
            // an error
            return Err(AppError::Config(
...
pub fn parse_bool(name: &str) -> Result<Option<bool>, AppError> {
   let Some(raw) = env::var(name).ok() else {
      return Ok(None);
   };
