...
pub async fn get_video_quality_observed(
   ...
   let result = (StatusCode::OK, Json(build_quality_observed_response(obs))).into_response();
   drop(observations);
   result
}
...
pub fn observe_quality_from_request(
   ...
   let Ok(mut observations) = state.quality_observations.lock() else {
      return;
   };
   ...
   let obs = observations.entry(video_id.clone()).or_insert_with(|| {
      QualityObservation {
   ...
   obs.current_itag.clone_from(&itag);
