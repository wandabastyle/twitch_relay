(function () {
  ...
  function currentAspectRatio() {
    ...
    const ratioText = (
      ...
      getComputedStyle(videoContainer).aspectRatio ||
      "16 / 9"
    ).trim();
  ...
  function syncPlayerLayout() {
    ...
    const shellPadX =
      readNumericStyle(watchShell, "padding-left") + readNumericStyle(watchShell, "padding-right");
    const shellPadY =
      readNumericStyle(watchShell, "padding-top") + readNumericStyle(watchShell, "padding-bottom");
  ...
  function updateDebug() {
    ...
    debugOverlay.innerHTML =
      '<div style="margin-bottom:8px;font-weight:bold;">Debug (Shift+D)</div><div>Quality: ' +
      currentQuality +
      "</div><div>Levels: " +
      (hlsInstance ? hlsInstance.levels.length : 0) +
      "</div><div>currentTime: " +
  ...
  function groupedPickerEmotes() {
    ...
    for (const item of filtered) {
      ...
      const title =
        typeof item.group_name === "string" && item.group_name.trim().length > 0
  ...
  async function ensureEmotesLoaded() {
    ...
    const response = await fetch(
      "/api/chat/emotes?channel_login=" + encodeURIComponent(chatChannel),
    ...
    const payload = await response.json();
    availableEmotes = (Array.isArray(payload && payload.emotes) ? payload.emotes : [])
  ...
  function appendChatEvent(event) {
    ...
    if (
      event.kind === "message" &&
      typeof event.sender_color === "string" &&
      event.sender_color.trim().length > 0
    )
    ...
    if (parts.length > 0)
      for (const part of parts) {
        if (part && part.kind === "emote" && typeof part.id === "string") {
          ...
          img.src =
            typeof part.image_url === "string" && part.image_url.trim().length > 0
              ? part.image_url
              : emoteUrl(part.id);
  ...
  if (chatNewMessagesPill)
    chatNewMessagesPill.addEventListener("click", function () {
      chatMessages.scrollTop = chatMessages.scrollHeight;
      clearUnreadChat();
    });
