...
export const recordingChannelOptions = (
  ...
  return channels.sort((first: string, second: string) => {
    if (first < second) {
      return SORT_LESS;
    }
    if (first > second) {
...
export const filterRecordingsByChannel = <
  EntryType extends { readonly channel_login: string },
>(
...
export const shownRecordingEntries = <
  EntryType extends { readonly channel_login: string },
>(
  entries: readonly EntryType[],
  channelFilter: string,
